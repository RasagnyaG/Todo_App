...
import { Change, Delete, LoginInput, NewStudent } from "../inputs/student";
...
class StudentResolver {
  ...
  @Authorized()
  async update(
    @Ctx() { student }: MyContext,
    @Arg("Changes") updatedInfo: Change
  ) {
    try {
      if (updatedInfo.email) student.email = updatedInfo.email;
      if (updatedInfo.name) student.name = updatedInfo.name;
      if (updatedInfo.password)
        student.password = bcryptjs.hashSync(updatedInfo.password);
  ...
  @Mutation(() => Boolean)
  async deleteStudent(@Arg("Id") { id }: Delete) {
    try {
      const student = await Student.findOneOrFail(id);
      const { affected } = await Student.update(id, { isHidden: true });
